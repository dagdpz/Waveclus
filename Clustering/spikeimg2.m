function spikeimg2(spikes2,cluster_class,ybounds)
...
for j = 0:dummy
    ...
    sf = get(gca,'XLim');
    sf = sf(2)/(c+sf(1));
    set(gca,'XTick',floor(sf*(0:20:c)));
    set(gca,'XTickLabel',num2cell(-40:20:c-40),'XGrid','on','YDir','normal');
    % calculate and set the YTicks
    yl = get(gca,'ylim');
    imrange = yl(1):yl(2);
    sf = yl(2)/(shift+ul);
    zerotick = round((shift+1)*sf);
