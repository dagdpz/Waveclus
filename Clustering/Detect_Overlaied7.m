...
files = dir('times_*.mat');
filesB = {files.name};
for m=1:length(filesB);
    tic
    filename = filesB{1,m};
    load(filename,'spikes','cluster_class','par','thr');
    handles.par = par;
...
%     channel=channels{m};
%     load(channel)
    load(['datahighpass' filename(end-8:end)])
    clear dataold
    data = double(data);
    cluster_classpre = [];
    spikespre = [];
...
%     group = [];
    ...
    print('-djpeg', ['datahighpassA' filename(14:end-4)])
    close
    ...
    set(gcf, 'PaperPositionMode', 'auto');
    print('-djpeg', ['datahighpassB' filename(14:end-4)])
    close
    ...
    set(gcf, 'PaperPositionMode', 'auto');
    print('-djpeg', ['datahighpass' filename(14:end-4)])
    close   
        
    end
    
%     save([filename(1:5) '_datahighpass' filename(14:end)],'cluster_class','spikes');
    save([filename(1:5) 'new_datahighpass' filename(end-6:end)],'cluster_class','spikes');
