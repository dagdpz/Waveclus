function [inspk,feature_names] = wave_features_new2(spikes,handles)
...
switch feature
   ...
   case 'wavpca',
      %wavelet
       cc=zeros(nspk,len);
            for i=1:nspk                                % Wavelet decomposition
                [c,l]=wavedec(spikes(i,12:54),scales,handles.par.wavelet);
                cc(i,1:len/2)=c(1:len/2);            %def.cc(i,1:ls)=c(1:ls);
                [c,l]=wavedec(spikes(i,64+1:end-10),scales,handles.par.wavelet);
                cc(i,len/2+1:len)=c(1:len/2);
            end
        %pca
      [C,S] = princomp(spikes(:,12:54));
      cc(1:nspk,len+1:len+3) = S(:,1:3);
      [C,S] = princomp(spikes(:,64+1:end-10));
