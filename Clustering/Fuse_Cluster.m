...
for m= 1:length(filesB);
    ...
    dummy = clustofuse(str2double(filename(end-6:end-4)),:);
...
%                 cluster_classnew(cluster_classnew(:,1) == ,1) = 
    ...
    if dummy > 5
        ...
        print('-djpeg', ['datahighpassnewA' filename(end-6:end-4)])
        close
        ...
        set(gcf, 'PaperPositionMode', 'auto');
        print('-djpeg', ['datahighpassnewB' filename(end-6:end-4)])
        close
        ...
        set(gcf, 'PaperPositionMode', 'auto');
        print('-dpng', ['datahighpassnew' filename(end-6:end-4)])
        close
    ...
    print('-dpng', ['datahighpassnewhist' filename(end-6:end-4)])
    close
