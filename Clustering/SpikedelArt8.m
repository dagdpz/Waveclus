function SpikedelArt8(badchan,handles)

numchan = handles.numchan;
numArray = handles.numArray;
dtype = handles.dtypewrite;
...
for j = 1 : numArray
    ...
    dataall=zeros(datalen,length(chanind),dtype);
    ...
    for i = 1 : ceil(datalen/chunks)
        if i == ceil(datalen/chunks)
            ...
            for k = 1 : length(chanind)
                fwrite(fid_dummy(k + dummyoffset), dataFilt(k,:),dtype);
            end
...
for i = 1 : length(chanindall)
    ...
    data = fread(fid_dummy(i),[dtype '=>' dtype]);
    fclose(fid_dummy(i));
