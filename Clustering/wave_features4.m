function [inspk,feature_names,inputs] = wave_features4(spikes,handles)
%Calculates the spike features

scales = handles.par.scales;
feature = handles.par.features;
exclusionmeth = handles.par.exclusionmeth; 
exclusioncrit = handles.par.exclusioncrit;
...
if handles.par.criticalvaluerejection
    crit = nan(100,1);
    for i = 1 : 100
...
switch exclusionmeth
    case 'corr'
        simmat = corr(ccall).^2 >= exclusioncrit;
    ...
    case 'euclidean'
        simmat = squareform(pdist(ccall','seuclidean')/sqrt(size(ccall,1))) <= exclusioncrit; %sqrt(2) + sqrt(1/size(ccall,1))*1.96; % correspond to 95% conf intervall for standart normal distributed noise difference
        simmat(diag(true(1,size(ccall,2)))) = 0;
...
if size(ccall,2) > maxinputs
    ...
    feature_names={fnall{1:size(ccall,2)}};
    inspk=ccall;
    inputs = size(ccall,2);
end
