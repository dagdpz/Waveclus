...
switch get(handles.htoread,'Value'),
    case 1, %ION
        t1=strfind(filename,'ch')+1;%minus is used to divide bname from channel
...
if isfield(q,'spikes')
    ...
    %--------
    handles.spikesadd=q.spikesadd;
    handles.spikescomp=q.spikecomp;
    %-------
else
...
tens_fname=sprintf('datahighpassch%s.mat',handles.filename(end-2:end));
