...
switch get(handles.htoread,'Value'),
    case 1, %ION
        t1=strfind(filename,'-');%minus is used to divide bname from channel
        t2=strfind(filename,'.mat');
        bname=filename(7:t1(1)-1);%skip word times
        handles.bname=bname;
        ch=str2double(filename(t1(end)+1:t2(1)-1));
        handles.channel=ch;
        handles.filename=sprintf('%s-%d',bname,ch);
    case 2, %UCLA
end
set(handles.textStatus,'string',sprintf('Loading %s',handles.filename),'fontsize',14);

%loading data, for now quick and dirty, i.e. only for ION

timesfile=sprintf('times_%s.mat',handles.filename);
% t=cputime-t; fprintf('start loading times file \t\t%1.0f\n',t); t=cputime;

q=load(timesfile);
% t=cputime-t; fprintf('end loading times file \t\t%1.0f\n',t); t=cputime;
...
tens_fname=sprintf('tens_%s.mat',handles.filename);
if ~exist(tens_fname,'file'), 
    ...
    q=load(tens_fname); 
    handles.ts=q.x;
    handles.ts_time=(1:length(q.x))/q.sr; 
...
handles.hexit=uicontrol('units','normalized','Style','pushbutton','String','Exit','FontSize',12,...
    'Tag','exit',...
    'UserData',handles.mainfig,...
    'Position',[stepx+(width+stepx)*4+0.1 1-(stepy+hight)*( 3 )-stepy*0.9 0.05 0.035],...
...
handles.hplotfeatures=uicontrol('units','normalized','Style','pushbutton','String','Features','FontSize',12,...
    'Tag','plotfeatures',...
    'UserData',handles.mainfig,...
    'Position',[stepx 1-(stepy+hight)*( 1 )-stepy*0.9 0.07 0.03],...
...
handles.htoplot = uicontrol('units','normalized','Style','popupmenu','FontSize',12,...
    ...
    'Value',3,...
    'UserData',handles.mainfig,...
    'Position',[stepx+0.08 1-(stepy+hight)*( 1 )-stepy*0.9 0.07 0.03],...
