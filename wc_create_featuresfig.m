...
if ishghandle(handles.mainfig) && isfield(handles,'hfeatures') && ishghandle(handles.hfeatures) %gui open, timecoursefigurehandle exists and figure is there
...
for i=2 %:nf,
    for j=i:nf,
        ...
        handles.hfaxes(k)=axes('position',[stepx+(width+stepx)*mod(k-1,ncol) 1-(stepy+hight)*(ceil(k/ncol)) width hight],...
            'Tag',sprintf('Feature1vs%d',j-2),'UserData',handles.mainfig,...
            'xtick',[],'ytick',[],'xlim',[mnx mxx],'ylim',[mny mxy],...
            'Parent',handles.hfeatures,'ButtonDownFcn',{@wc_cluster_cut_callback},'NextPlot','add');
        if j==i
