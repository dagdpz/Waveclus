...
if numel(to_fuse)>1
    ...
    handles.fixed(end)=0;
    set(handles.hfix(end),'Value',0);
    handles.ncl=handles.ncl-numel(to_fuse);
end
...
%function printbutton_Callback(source,~)
% handles=guidata(get(source,'UserData'));
% set(handles.textStatus,'string',sprintf('Saving %s',handles.filename));
% handles=wc_saveresults(handles);
% figure(handles.mainfig);
...
% handles.hprint=uicontrol('units','normalized','Style','pushbutton','String','Print','FontSize',12,...
%     'Tag','print',...
%     'UserData',handles.mainfig,...
%     'Position',[stepx+(width+stepx)*4 1-stepy*0.9 0.05 0.035],...
%     'Callback',{@printbutton_Callback});
...
handles.htempmatch=uicontrol('units','normalized','Style','togglebutton','String','Nearest','FontSize',12,...
    'Tag','Nearest',...
    'UserData',handles.mainfig,...
    'Position',[stepx+0.04 1-(stepy+hight)*( nrow )-stepy*0.9 0.05 0.035],...
...
handles.hfuseclusters=uicontrol('units','normalized','Style','togglebutton','String','Fuse','FontSize',12,...
    'Tag','fuse',...
    'UserData',handles.mainfig,...
    'Position',[stepx+0.09 1-(stepy+hight)*( nrow )-stepy*0.9 0.04 0.035],...
    'Callback',{@fusebutton_Callback});
handles.hexit=uicontrol('units','normalized','Style','pushbutton','String','Exit','FontSize',12,...
    'Tag','exit',...
    'UserData',handles.mainfig,...
    'Position',[stepx+0.13 1-(stepy+hight)*( nrow )-stepy*0.9 0.04 0.035],...
...
for i=1:ncol*2,
    j=i+12;
    x=stepx+(width+stepx)*mod(i-1,ncol);
    y=1-(stepy+hight)*( (i>ncol)*2+2 )-stepy*0.75;
