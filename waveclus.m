...
handles.hfuseclusters=uicontrol('units','normalized','Style','togglebutton','String','Fuse','FontSize',12,...
    'Tag','fuse',...
    'UserData',handles.mainfig,...
    'Position',[stepx+0.09 1-(stepy+hight)*( nrow )-stepy*0.9 0.06 0.035],...
...
for r=2:nrow
    for c=1:ncol-1,
        ...
        handles.spikeaxes(i)=axes('position',[x y+stepy*0.45 width hight],...
            'Tag',sprintf('Clust%d',i),'ButtonDownFcn',{@wc_cluster_cut_callback},'NextPlot','add');
...
for r=1:nrow
    for c=1:ncol,
        ...
        handles.spikeaxes(j)=axes('position',[x y+stepy*0.75 width hight],...
            'Tag',sprintf('Clust%d',j),'ButtonDownFcn',{@wc_cluster_cut_callback});
